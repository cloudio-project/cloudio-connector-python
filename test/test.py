...
class Tester(AttributeListener):
    def __init__(self):
        ...
        attr = [
            AttributeId(uuid=cc.get_uuid('myEndpoint'), node='node', objects=['object'], attribute='attr'),
            AttributeId(uuid=cc.get_uuid('myEndpoint'), node='node', objects=['object'], attribute='attr2')]
        cc.subscribe_to_attributes(attr)
